use crate::{
    ...
    primitives::{Answers, CliOptions, GlobalData, ResponseTime, TeamAnswers},
    utils::parser::{parse_and_add_tags, parse_answers},
};
...
pub async fn collect_team_data(
    questions: Vec<Item>,
    members: &Vec<u32>,
    options: &CliOptions,
) -> Answers {
    ...
    for question in &questions {
        if question.is_answered.unwrap()
            || (!question.is_answered.unwrap() && question.answer_count.unwrap() > 0)
        {
            let mut response_time: ResponseTime =
                ResponseTime::new(question.creation_date, 0, false);

            let answers: APIResponse =
                stackexchange_api::get_answers(question.question_id, &options.site).await;

            let team_answers: TeamAnswers = parse_answers(
                answers,
